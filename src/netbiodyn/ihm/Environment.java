...
//    private ArrayList<elementCourbe> _courbes = new ArrayList<>();
    ...
    private void dataGridView_entitesMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_dataGridView_entitesMouseClicked
        if (evt.getClickCount() == 2 && !evt.isConsumed() && !freezed) {
            ...
            if ((dataGridView_entites.getSelectedIndex() > 0) && (dataGridView_entites.getSelectedIndices().length <= 1)) {
                String etiquette = UtilDivers.str_originale(getDataGridView_entites().getModel().getElementAt(dataGridView_entites.getSelectedIndex()).toString());
                if (etiquette.compareTo(lastSelected) == 0) {
                    dataGridView_entites.clearSelection();
                    lastSelected = "";
                    wasSelected = true;
                } else {
                    lastSelected = etiquette;
                }
            }
        }
    ...
    private void button_playMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_button_playMouseClicked
        controller.play();
//        controller.playUntilThenExport(fixedStep, "C:\\Users\\riviere\\Desktop\\Simulation Biologie\\Projet Abeilles\\Stage 01-02 2016\\Résultats simulations abeilles\\ModeleSaisons2602-3.csv");
    ...
    private void memoriserCourbes() {
        for (Agent proto : _ListManipulesNoeuds) {
            ...
            curves.addPoint(proto._etiquettes, proto.Couleur, _time * 1.0, nb_p * 1.0);
        }
