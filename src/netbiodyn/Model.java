...
//                }
                ...
                for (int i = 0; i < delta; i++) {
                    instances.removeReaxel(removable.get(i));
                    size = size - 1;
                }
            }
        }
    ...
    public boolean load(Environment env, String nomFichier, JProgressBar waiter) {
        sl = new FileSaverLoader(env, nomFichier);

        Serialized saved = sl.load(waiter);
        if (saved != null) {
            clearEnvironment();
    ...
    public boolean save(Environment env, String nomFichier, String path, JProgressBar waiter) {
        sl = new FileSaverLoader(env, nomFichier);
        ...
        return sl.saveToSerial(toSave, waiter);
    }
